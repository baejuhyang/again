...
// 두 매개변수 'a', 'b' 모두 number, 반환 값도 number type
...
// ㄴ 리턴이 없거나 함수가 단순히 작업을 수행만 할 때 쓰는 타입 명
print(2, 4, 6); // 2 4 6
...
// goingOn();
// - 무한루프나 예외처리가 필요한 경우 never 타입으로 명시하여 실행하지 못하게 작성.

// #10. 오버라이딩 vs 오버로딩
// ** 오버라이딩
// - 클래스에서 부모 클래스의 메서드를 자식 클래스에서 '재정의'하는 개념
// - 메서드의 이름과 '매개변수 목록이 동일'하며, 자식클래스에서 새로운 구현을 제공하여 부모 클래스의 동작을 변경.
class Animal {
    speak(): void {
...
// - 같은 기능을 여러 방법으로 수행해야 할 때 보통 사용한다.
// Ex1)
...
console.log(greet('Una')); // Hello Una.
