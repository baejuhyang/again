...
const reducer = (prevState, action) => {
    ...
    console.log('현재 상태의 값', prevState.value); // 0

    // action = {type: xx} 형태로 들어옴
    console.log('action', action); // {type: 'INCREMENT'}
    console.log('action의 type', action.type);

    switch (action.type) {
        case 'INCREMENT': //대문자로 쓰는 것이 관례
...
// - 업데이트 로직을 컴포넌틔 외부로 뺌!
// 컴포넌트에 속해있지 않기 때문에, 위의 내용 자체를 재사용도 가능
export default function UseReducerEx() {
    // #3. useReducer 훅 사용
    const [state, dispatch] = useReducer(reducer, initState);
    // state: 현재 상태
    // dispatch: 액션을 발생시키는 함수 (=state가 어떻게 변경되어야 하는지에 대한 힌트 제공)
    // reducer: state를 업데이트 하는 함수.

    console.log('state 초기 상태', state); // {value: 0}

    // #4. 액션 핸드럴 함수 정의
    // - 이 함수들은 'dispatch'를 호출하여 액션을 발생 시킴.
    // - dispatch는 액션 객체를 인수로 받으며 {type: 'ACTION_TYPE', payload: data} 형태
    // * payload: 액션 객체에 담아 보내는 추가적인 데이터 (숫자, 문자열, 객체 등 모든 형태 데이터 가능)
