...
import './App.css';
import image from './assets/donut.jpg';
...
function App() {
    ...
    return (
        <div className="App">
            ...
            {/* 
              - {} 로 감싸면 js 표현식 사용 가능
              - {} 안에서 삼항 연산자 사용 가능 (if, for문은 사용 불가)
            */}
            <div>{name} 안녕?</div>
            <div>
                {student === 'sesac' ? (
            ...
            {/* #3. style 속성 */}
            {/* 
            - 리액트에서 Dom 요소에 스타일 적용 시 문자열 안됨. 객체를 사용!
            - {{스타일명: '값'}} 
            - 스타일 이름 중 하이픈(-) 포함 시 camelCase로 작성해야 함. JS에서는 -을 진짜 빼기로 오인할 수 있음.
              - ex) backgroundColor, fontSize
            */}
            ...
            {/* #5. 종료 태그 closing tag가 없는 태그 사용 시 꼭 작성 해줘야 함. */}
            {/* - 기존의 종료 태그가 없는 태그 종류
            ...
            */}
            ...
            {/* src 내부의 이미지를 사용할 경우 -> 맨 위에서 이미지를 import 해야함. */}
            <img src={image} />
        </div>
    );
}
